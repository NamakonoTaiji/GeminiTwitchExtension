...
// デバウンス関数の実装 - 指定時間内の複数回の呼び出しを1回にまとめる
...
function handleChannelChange(prevUrl, currentUrl) {
  ...
  // URLからチャンネル名を抽出
  const oldChannelMatch = prevUrl.match(/twitch\.tv\/(\w+)/);
  ...
  // チャンネル切り替え時のフラグ確認
  console.log("チャンネル変更処理内でフラグを設定");
  sessionStorage.setItem("twitch_gemini_channel_changed", "true");
  sessionStorage.setItem("twitch_gemini_prevent_existing", "true");
  ...
  gracePeriodTimer = setTimeout(() => {
    ...
    console.log(
      "グレースピリオド終了: チャンネル切り替え後の初期ロードが完了したと判断します"
    );
    ...
    if (observer) {
      ...
      if (chatContainer) {
        ...
        console.log(
          `${existingMessages.length}個のメッセージを既存メッセージとしてマーク完了`
        );
      }
    }
...
const debouncedReinitialize = debounce(async () => {
  // チャンネル変更フラグをチェック（デバウンス中に変更がある可能性があるため）
  const channelChangedFlag = sessionStorage.getItem(
    "twitch_gemini_channel_changed"
  );
  if (channelChangedFlag === "true") {
    console.log(
      "デバウンス処理中のチャンネル変更を検出しました。既存メッセージ処理を確実に無効化します。"
    );
  }
  ...
  // 現在の翻訳状態をログ出力
  console.log(
    `翻訳状態確認 - 有効: ${isEnabled}, APIキー: ${
      apiKeySet ? "設定済み" : "未設定"
    }`
  );
  console.log(
    `既存メッセージ処理設定: ${
      settings.processExistingMessages ? "有効" : "無効"
    }`
  );
  ...
  console.log(
    `チャンネル変更に伴い翻訳済みキャッシュをクリア (${previousSize} エントリ)`
  );
  ...
  // 変更前の設定をログ出力
  console.log(
    `チャンネル切り替え時の既存メッセージ処理を強制無効化 (元設定: ${originalSetting})`
  );

  // 設定を一時的に置き換え
  const tempSettings = { ...settings };
  tempSettings.processExistingMessages = false;
  sessionStorage.setItem("twitch_gemini_prevent_existing", "true"); // 既存メッセージ処理禁止フラグもセット
...
function startUrlChangeDetection() {
  ...
  urlCheckInterval = setInterval(() => {
    ...
    if (currentUrl !== lastUrl) {
      ...
      // デバウンスされたチャンネル変更通知を実行
      // まずチャンネル変更関連のフラグをセット（デバウンス前に設定して確実に捕捉）
      sessionStorage.setItem("twitch_gemini_channel_changed", "true");
      sessionStorage.setItem("twitch_gemini_prevent_existing", "true");
      console.log(
        "チャンネル変更フラグと既存メッセージ処理禁止フラグをセットしました"
      );
...
async function initialize() {
  ...
  try {
    ...
    console.log(
      `設定を読み込みました: 有効=${isEnabled}, APIキー設定済み=${apiKeySet}`
    );
    ...
    if (
      ...
      console.warn(
        "拡張機能コンテキストが無効になりました。ローカル設定を使用します。"
      );
      ...
      try {
        ...
        chrome.runtime.sendMessage({ action: "getSettings" }, (response) => {
          if (chrome.runtime.lastError) {
            ...
            if (localSettings) {
              console.log(
                "バックグラウンドからの設定取得に失敗しました。ローカル設定を使用します"
              );
            ...
            try {
              localStorage.setItem(
                "twitch_gemini_settings",
                JSON.stringify(response)
              );
              console.log(
                "バックグラウンドから設定を取得し、ローカルに保存しました"
              );
            } catch (storageError) {
        ...
        if (localSettings) {
          console.log(
            "バックグラウンドへのメッセージ送信に失敗しました。ローカル設定を使用します"
          );
          resolve(localSettings);
        } else {
          console.log("ローカル設定がないため、デフォルト設定を使用します");
...
function startObserving() {
  ...
  if (sessionStorage.getItem("twitch_gemini_channel_changed") === "true") {
    console.log(
      "❌ チャンネル切り替え検出: 既存メッセージ処理禁止フラグを設定"
    );
    sessionStorage.setItem("twitch_gemini_prevent_existing", "true");
  }
...
function observeChatMessages(container) {
    ...
    addedNodes.forEach((node, index) => {
      setTimeout(() => {
        ...
        if (!isInGracePeriod) {
          ...
          console.log("新規メッセージを検出しました");
        } else {
          node._isExistingMessage = true;
          console.log(
            "グレースピリオド中のメッセージを既存メッセージとして分類します"
          );
        }
  ...
  if (sessionStorage.getItem("twitch_gemini_channel_changed") === "true") {
    console.log(
      "チャンネル切り替え後の監視開始: 既存メッセージ処理を確実に無効化"
    );
  ...
  // 強制的に既存メッセージ処理を無効化
  console.log(
    "既存メッセージ処理を強制無効化します (フラグ状態:",
    channelChangedFlag === "true" ? "チャンネル変更あり" : "チャンネル変更なし",
    ")"
  );
  shouldProcessExisting = false;
  ...
  // チャンネル切り替え時は、既存メッセージ処理禁止フラグを永続的に維持する
  // 監視開始から3秒後に既存メッセージ処理禁止フラグをクリアする処理は行わない
  /*
  setTimeout(() => {
    if (sessionStorage.getItem("twitch_gemini_prevent_existing") === "true") {
      console.log('既存メッセージ処理禁止フラグをクリアします');
      sessionStorage.removeItem("twitch_gemini_prevent_existing");
    }
  }, 3000);
  */
  ...
  console.log(
    `既存メッセージ処理設定: ${
      shouldProcessExisting ? "有効" : "無効"
    } (元の設定: ${settings.processExistingMessages ? "有効" : "無効"})`
  );
  ...
  console.log(
    `既存メッセージ処理は${
      shouldProcessExisting ? "有効" : "無効"
    }です (ユーザー設定: ${settings.processExistingMessages ? "有効" : "無効"})`
  );

  // チャンネル切り替え時は、一律で既存メッセージ処理を無効化
  if (
    location.href !== lastUrl ||
    sessionStorage.getItem("twitch_gemini_prevent_existing") === "true"
  ) {
    console.log(
      "⚠️ チャンネル切り替え検出または明示的な禁止フラグあり: 既存メッセージ処理を無効化します"
    );
    shouldProcessExisting = false;
  }
...
async function processChatMessage(messageNode) {
  ...
  if (!messageNode._isNewMessage || messageNode._isExistingMessage) {
    ...
    if (
      ...
      sessionStorage.getItem("twitch_gemini_channel_changed") === "true" ||
      sessionStorage.getItem("twitch_gemini_prevent_existing") === "true"
    ) {
      return;
    }
...
function shouldTranslate(text) {
  ...
  if (japaneseRatio >= japaneseThreshold) {
    console.log(
      `日本語率: ${(japaneseRatio * 100).toFixed(1)}% - 翻訳しません`
    );
    return false;
  }
...
const debouncedHandleContextInvalidated = debounce(() => {
  ...
  const contextInvalidatedFlag = sessionStorage.getItem(
    "twitch_gemini_context_invalidated"
  );
  const now = Date.now();
  ...
  // 再試行回数を増やす
  const retryCount =
    parseInt(sessionStorage.getItem("twitch_gemini_retry_count") || "0") + 1;
...
(() => {
  ...
  function checkExtensionContext() {
    try {
      ...
      // 再初期化の試行回数のカウント
      const retryCount =
        parseInt(sessionStorage.getItem("twitch_gemini_retry_count") || "0") +
        1;
      sessionStorage.setItem(
        "twitch_gemini_retry_count",
        retryCount.toString()
      );
